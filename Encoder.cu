...
void ldpcEncoder (unsigned int *messageBits, unsigned int* W_ROW_ROM,
                  ...
                  unsigned int shiftRegLength, unsigned int *codeWord)  {
  ...
  printf ("numMsgBits %i, numParBits %i, shiftRegLength %i, numShiftRegs %i, numRowsInRom %i\n",
          numMsgBits, numParBits, shiftRegLength, numShiftRegs, numRowsInRom);
...
// Loop on the number of rows in the ROM.
  for (unsigned int romRowIndex = 0; romRowIndex< numRowsInRom; romRowIndex++) {
    ...
    for (unsigned int dummyIndex = 0; dummyIndex < shiftRegLength; dummyIndex++) {
